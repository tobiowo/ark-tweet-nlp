...
public class Tagger {
    ...
    public static void main(String[] args) throws IOException {
        ...
        BufferedReader br;
        if (args.length > 0)
            br = BasicFileIO.openFileToReadUTF8(args[0]);
        else
            br = new BufferedReader(new InputStreamReader(System.in, "UTF-8"));
        while ((text = br.readLine()) != null) {
            List<TaggedToken> taggedTokens = tagger.tokenizeAndTag(text);
            if (taggedTokens == null)   continue;
            StringBuilder accum = new StringBuilder();
            String lastToken = "";
            String lastTag = "";
            for (TaggedToken token : taggedTokens) {
                if (token.tag.equals("A")
                        || token.tag.equals("N") || token.tag.equals("^")
                        || token.tag.equals("D") || token.tag.equals("S") || token.tag.equals("$")
                        || (lastTag != null && lastTag.equals("N") && token.token.equalsIgnoreCase("of"))
                        || (lastToken != null && lastToken.equalsIgnoreCase("of") && token.token.equalsIgnoreCase("the"))
                        ) {
                      accum.append(token.token).append(" ");
                    } else {
                      if (accum.length() > 0) {
                        accum.deleteCharAt(accum.length() - 1);
                        System.out.print(accum.toString() + "\t");
