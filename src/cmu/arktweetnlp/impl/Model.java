...
public class Model {
     ...
     * rolling edge feature extraction
     **/
    public void easyDecode(ModelSentence sentence, boolean storeConfidences) {
        int T = sentence.T;
        HashSet<Integer> unprocessed = new HashSet<Integer>();
        sentence.labels = new int[T];
        Arrays.fill(sentence.edgeFeatures, Integer.MIN_VALUE);
        if (storeConfidences)
            sentence.confidences = new double[T];
        ...
        for (int t = 0; t < T; t++) {
            unprocessed.add(t);
            computeEasyLabelScores(t, sentence, labelScores[t]);
        }
        ...
        while (!unprocessed.isEmpty()) {
            ...
            for (int t : unprocessed) {
                computeEasyEdgeScores(t, sentence, labelScores[t]);
                Arrays.fill(sentence.edgeFeatures, Integer.MIN_VALUE);
                int tag = ArrayMath.argmax(labelScores[t]);

                double[] ls = labelScores[t].clone();
                ArrayMath.expInPlace(ls);
                double Z = ArrayMath.sum(ls);
                ArrayMath.multiplyInPlace(ls, 1.0 / Z);
                //ArrayMath.logNormalize(ls);
                double score = ls[tag];
/*                if (t == 0) {   //first then easy first?
                    bestIndex = t;
                    bestTag = tag;
                    bestScore = score;
                    break;
            ...
            sentence.labels[bestIndex] = bestTag;
            if (bestIndex < T - 1)
                sentence.edgeFeatures[bestIndex + 1] = sentence.labels[bestIndex];
            unprocessed.remove(bestIndex);
            System.err.println(bestIndex+"\t"+labelVocab.name(bestTag)+"\t"+bestScore);
            if (storeConfidences) {
                ArrayMath.expInPlace(labelScores[bestIndex]);
                double Z = ArrayMath.sum(labelScores[bestIndex]);
                ArrayMath.multiplyInPlace(labelScores[bestIndex], 1.0 / Z);
                sentence.confidences[bestIndex] = labelScores[bestIndex][bestTag];
            }
        }
    }
     ...
     */
    public void viterbiDecode(ModelSentence sentence, boolean storeConfidences) {
        int T = sentence.T;
        sentence.labels = new int[T];
        ...
        if (storeConfidences) {
            ArrayUtil.logNormalize(vit[T - 1]);
            sentence.confidences[T - 1] = Math
                    .exp(vit[T - 1][sentence.labels[T - 1]]);
        }
        ...
        for (int i = T - 2; (i >= 0) && (backtrace != startMarker()); i--) { // termination
            ...
            if (storeConfidences) {
                ArrayUtil.logNormalize(vit[i]);
                sentence.confidences[i] = Math.exp(vit[i][backtrace]);
            }
    ...
    public void computeEasyLabelScores(int t, ModelSentence sentence,
            double[] labelScores) {
        sentence.edgeFeatures[0] = startMarker();
        computeBiasScores(labelScores);
        computeEasyEdgeScores(t, sentence, labelScores);
        computeObservedFeatureScores(t, sentence, labelScores);
    }
    ...
    /**
     * Adds into labelScores. edgeFeatures may be INT_MIN: if so, ignore it.
     * **/
    public void computeEasyEdgeScores(int t, ModelSentence sentence,
            double[] labelScores) {
        // Util.p(sentence.edgeFeatures);
        int prev = sentence.edgeFeatures[t];
        ...
        if (t < sentence.T-2) {
            int next = sentence.edgeFeatures[t+2];
            if (next != Integer.MIN_VALUE) {
                for (int k = 0; k < numLabels; k++) {
                    labelScores[k] += edgeCoefs[k][next];
                }
            }
        }
