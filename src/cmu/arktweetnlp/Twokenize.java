 ...
 * European languages. This tokenizer code has gone through a long history:
 * 
 ...
 * many improvements (2012-06) Added support for full width symbols to better
 * handle Han script Current home is http://github.com/brendano/ark-tweet-nlp
 * and http://www.ark.cs.cmu.edu/TweetNLP
 * 
...
public class Twokenize {
    static Pattern Contractions = Pattern
            .compile("(?i)(\\w+)(n['’′]t|['’′]ve|['’′]ll|['’′]d|['’′]re|['’′]s|['’′]m)$");
    ...
    static String bfLeft = "(♥|0|o|°|v|\\$|(?<![Rr])t|x|;|\u0CA0|@|ʘ|•|・|◕|\\^|¬|\\*|ˆ|˘|✖|\\+)";
    static String bfCenter = "(?:[\\.‿дД▽︵∀]|[_-]+|[o0](?!o))";
    static String bfRight = "\\2";
    static String s3 = "(?:--['\"])|\\u30C4";
    static String s4 = "(?:<|&lt;|>|&gt;|¬)[\\._-]+(?:<|&lt;|>|&gt;|¬)['\"]?";
    static String s5 = "(?:[.][_]+[.])";
    static String basicface = "(?:(?i)" + bfLeft + bfCenter + bfRight + ")|"
            + s3 + "|" + s4 + "|" + s5;
    //mouths = дД▽︵‿
    static String eeLeft = "(?:¯\\\\_)?[＼\\\\ƪԄ\\(（<>;ヽ\\-=~\\*φ\\p{Ps}]+";
    static String eeRight = "[\\-=\\);'\\u0022<>ʃ）/／ノﾉ丿╯σっµ~\\*\\p{Pe}]+(?:_/¯)?";
    static String eeSymbol = "[^A-Za-z0-9\\s\\(\\)\\*:=-]";     //TODO:change
    static String eastEmote = eeLeft + "(?:" + basicface + "|" + eeSymbol
            + ")+" + eeRight;
    ...
    static String Hashtag = "[#＃][\\p{L}]+"; // optional: lookbehind for \b
    // optional: lookbehind for \b, max length 15
    static String AtMention = "[@＠][a-zA-Z0-9_]{1,15}";
    ...
    static String Han = "[\u4e00-\u9FFF]+";
    static String noSpaces = "[\\p{InCJK Unified Ideographs}\u0e00-\u0eff\\p{InKhmer}]";
    //Khmer, Lao, Han, and Thai scripts
    
    // We will be tokenizing using these regexps as delimiters
    // Additionally, these things are "protected", meaning they shouldn't be
    // further split themselves.
    private static Pattern Protected = Pattern.compile(OR(Hearts, url, Email,
            timeLike,
            // numNum,
            numberWithCommas, numComb, emoticon, Arrows, entity, punctSeq,
            arbitraryAbbrev, separators, decorations, embeddedApostrophe,
            Hashtag, AtMention, Han));
    ...
    static String offEdge = "(^|$|:|;|\\s|\\.|,|：|"+ noSpaces +")"; // colon here gets "(hello):"
                                                   // ==> "( hello ):"
    static Pattern EdgePunctLeft = Pattern.compile(offEdge + "(" + edgePunct
            + "+)(" + notEdgePunct + ")");
    static Pattern EdgePunctRight = Pattern.compile("(" + notEdgePunct + ")("
            + edgePunct + "+)" + offEdge);
    ...
    private static List<String> simpleTokenize(String text) {
        ...
        while (matches.find()) {
            ...
            if (matches.start() != matches.end()) { // unnecessary?
                ...
                bads.add(bad);
                badSpans.add(matches.start());
                badSpans.add(matches.end());
            }
        }
        ...
        // string
        List<List<String>> splitGoods = new ArrayList<List<String>>(
                indices.size() / 2);
        for (int i = 0; i < indices.size(); i += 2) {
            String goodstr = splitPunctText.substring(indices.get(i),
                    indices.get(i + 1));
    ...
    private static List<String> addAllnonempty(List<String> master,
            List<String> smaller) {
    ...
    public static Pattern getProtected() {
        return Protected;
    }
